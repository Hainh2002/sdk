...
static uint64_t sync_msg_100_cnt = 0;
static uint64_t sync_msg_num = 0;
...
void app_co_sync(bool _enable){
    if(!_enable){
        printf("turn off bp sync\n");
        od_temp_comm_profile_para.x1005_cob_id_sync_message.bit_30 = CO_SYNC_NOT_GEN;
//        CO_recover_sync_mode(&CO_DEVICE, &od_temp_comm_profile_para);
    }else{
        od_temp_comm_profile_para.x1005_cob_id_sync_message.bit_30 = CO_SYNC_GEN;
        printf("turn on bp sync\n");
